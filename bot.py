...
class AppointmentStatusTranscriptFallback(FrameProcessor):
    ...
    def _is_cancel(text: str) -> bool:
        ...
        if not t:
            return False
        return any(x in t for x in ["الغ", "إلغاء", "الغاء", "كنسل", "cancel", "مش بد", "مش بدي", "مش راي", "مش رايح", "ما بقدر", "ما بنفع"])
...
async def run_bot(websocket_client, appointment_data, call_control_id=None):
    ...
    clinic_name = appointment_data.get("clinic_name", "عيادة الأسنان")
    greeting_text = f"السلام عليكم، معك سارة، منسقة المواعيد من {clinic_name}. معي {patient_name}؟"
...
٧) لو سأل عن العنوان أو الوصول:
"العيادة موجودة في {appointment_data.get('clinic_address', 'عمّان')}. بنبعتلك رسالة فيها الموقع والتفاصيل."

٨) لو متأخر أو بدّه يأجل:
"لا مشكلة، ممكن نأجل. شو الوقت اللي بناسبك؟"
...
- عنوان العيادة: {appointment_data.get('clinic_address', 'عمّان')}
...
- استخدمي "حضرتك" للاحترام.
- خليكي إيجابية ومبتسمة بصوتك.
"""
    ...
    if use_multimodal_live:
        ...
        gemini_params = GeminiLiveInputParams(temperature=0.4)
