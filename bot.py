...
BOT_BUILD_ID = "2026-01-21-ammani-dental-ultimate"
_VAD_MODEL = {"value": None}

# -------------------------------------------------------------------------
# FRAME PROCESSORS (Helper Classes)
# -------------------------------------------------------------------------
...
async def run_bot(websocket_client, lead_data, call_control_id=None):
    ...
    # ────────────────────────────────────────────────
    # FINAL ULTRA-ENHANCED SYSTEM PROMPT
    # ────────────────────────────────────────────────
    system_prompt = f"""
# ROLE: سارة – أفضل منسقة علاج أسنان أردنية أصلية من عمان
...
"""
    ...
    if use_multimodal_live:
        ...
        # DEBUG: Confirm config before connecting
        logger.info(f"PRE-CONNECT CONFIG → model: {model} | voice: {voice_id} | sample_rate: {gemini_in_sample_rate} | LANGUAGE: NOT SET (auto-detect)")
        ...
        # ─── CRITICAL FIX: NO LANGUAGE CODE HERE ───
        # Removed all language setting to avoid 1007 error
        ...
        async def confirm_appointment(params: FunctionCallParams):
            ...
            # For demo/testing: comment real update
        ...
        async def _on_gemini_live_error(service, error):
            ...
            if "Unsupported language code 'ar-XA'" in msg or "Unsupported language code" in msg:
                logger.error("GeminiLive language mismatch – ensure no language code is set")
                logger.error(msg)
